...
Simple Todo list application using Django and AngularJS
...
Django Todo List relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'Django Todo List' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "Django Todo List <noreply@todolist.local>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "Django Todo List <noreply@todolist.local>"
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[Django Todo List] "
DJANGO_ALLOWED_HOSTS                    ALLOWED_HOSTS               ['*']                                          ['todolist.local']
======================================= =========================== ============================================== ======================================================================
...
To run the tests, enter the django_todolist/tests directory and run the following commands::
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=todolist.local` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/django_todolist/postgres` by default. Change that if you wan't
...
If you are using `supervisor`, you can use this file as a starting point::
    ...
    [program:django_todolist]
    command=docker-compose up
    directory=/path/to/django_todolist
...
Place it in `/etc/supervisor/conf.d/django_todolist.conf` and run::

    supervisorctl reread
    supervisorctl start django_todolist
